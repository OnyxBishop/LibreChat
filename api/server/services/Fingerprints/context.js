...
/**
 * Builds the fingerprint context block for an LLM prompt: explicitly named
 * participants (by id) merged with semantic matches from free text. Best-effort —
 * any failure (embedding provider down, bad id) yields an empty string so the
 * caller's main flow (a meeting suggestion, a chat turn) is never blocked.
 *
 * @param {object} params
 ...
 */
async function buildFingerprintContext({
  req,
  userId,
  participantIds = [],
  queryText = '',
  ...
  try {
    ...
    if (ids.length > 0) {
      const participants = await Promise.all(
        ids.map((id) =>
          getFingerprintById({ author: userId, id }).catch(() => null),
        ),
      );
    ...
    return formatFingerprintContext(Array.from(byId.values()));
  } catch (error) {
...
module.exports = { buildFingerprintContext };
